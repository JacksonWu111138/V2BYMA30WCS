...
{
    public partial class CtrlBufferInitial : Form
    {
        ...
        private void button_BufferInitial_Click(object sender, EventArgs e)
        {
            bool ctrltype = true;
            ...
            if (ctrltype)
            {
                BufferInitialInfo info = new BufferInitialInfo
                {
                    jobId = textBox_jobId.Text,
                    bufferId = textBox_bufferId.Text
                };
                if (!clsAPI.GetAPI().GetBufferInitial().FunReport(info, Apiconfig.IP))
                {
                    MessageBox.Show($"失敗, jobId:{info.jobId}.", "Buffer Initial", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show($"成功, jobId:{info.jobId}.", "Buffer Initial", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
    }
