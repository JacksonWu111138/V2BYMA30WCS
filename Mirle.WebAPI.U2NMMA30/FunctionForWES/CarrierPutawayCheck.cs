...
{
    public class CarrierPutawayCheck
    {
        public bool FunReport(CarrierPutawayCheckInfo info, string IP)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, strJson);
                string sLink = $"http://{IP}/CARRIER_PUTAWAY_CHECK";
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Debug, $"URL: {sLink}");
                string re = clsTool.HttpPost(sLink, strJson);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, re);
                var info_controller = (CarrierPutawayCheckReply)Newtonsoft.Json.Linq.JObject.Parse(re).ToObject(typeof(CarrierPutawayCheckReply));
