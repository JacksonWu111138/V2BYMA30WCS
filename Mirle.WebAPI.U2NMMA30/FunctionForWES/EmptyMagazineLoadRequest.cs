...
{
    public class EmptyMagazineLoadRequest
    {
        public bool FunReport(EmptyMagazineLoadRequestInfo info, string IP)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, strJson);
                string sLink = $"http://{IP}/EMPTY_MAGAZINE_LOAD_REQUEST";
