...
{
    public class EmptyESDCarrierLoadRequest
    {
        public bool FunReport(EmptyESDCarrierLoadRequestInfo info, string IP)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, strJson);
                string sLink = $"http://{IP}/EMPTY_ESDCARRIER_LOAD_REQUEST";
