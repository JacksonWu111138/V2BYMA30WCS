...
{
    public class CarrierShelfReport
    {
        public bool FunReport(CarrierShelfReportInfo info, string IP)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, strJson);
                string sLink = $"http://{IP}/CARRIER_SHELF_REPORT";
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Debug, $"URL: {sLink}");
                string re = clsTool.HttpPost(sLink, strJson);
                clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Trace, re);
                var info_controller = (CarrierShelfReply)Newtonsoft.Json.Linq.JObject.Parse(re).ToObject(typeof(CarrierShelfReply));
